...
class ReactionIndex(Entity):
    id = PrimaryKey(int, auto=True)
    cgrsmiles = Required(str, lazy=True)
    _fingerprint = Required(IntArray, lazy=True)
    ...
    def __init__(self, structure: ReactionContainer, /, **kwargs):
        ...
        super().__init__(cgrsmiles=str(cgr_structure), _fingerprint=fingerprint,
    ...
    def unload(self):
        self._vals_.pop(ReactionIndex._structure, None)
        self._vals_.pop(ReactionIndex.fingerprint, None)
        self._vals_.pop(ReactionIndex.cgrsmiles, None)
        self.__dict__.pop('structure', None)

    @staticmethod
    def __postprocess_exact_match(result):
        return ReactionIndex[result]
    ...
    def __postprocess_unordered_cgrs(result, *, fingerprint=None, substr=None):
        cgr = ReactionIndex[result[0]]
        fps = set(cgr._fingerprint)
        tanimoto = len(fps.intersection(fingerprint)) / len(fps.union(fingerprint))
        del cgr._vals_[ReactionIndex._fingerprint]
        ...
        if substr is None:
            ...
            del cgr._vals_[ReactionIndex._structure]
    ...
    def _similarity_unorderd(cls, fingerprint: list) -> RequestPack:
        ...
        return RequestPack(request, partial(cls.__postprocess_unordered_cgrs, fingerprint=fingerprint),
                           prefetch_map=(ReactionIndex, 0, [ReactionIndex.fingerprint]))
    ...
    def _similarity_ordered(cls, fingerprint: list) -> RequestPack:
        ...
        return RequestPack(request, cls.__postprocess_ordered_cgrs,
                           prefetch_map=(ReactionIndex, 0, None))

    def similars(self: Union[CGRContainer, "ReactionIndex"], ordered=True, request_only=False):
        if isinstance(self, ReactionIndex):
...
# substructure search block
    ...
    def _substructure_ordered(cls, fingerprint: list, substr=None) -> RequestPack:
        ...
        return RequestPack(request, partial(cls.__postprocess_ordered_cgrs, substr=substr),
                           prefetch_map=(ReactionIndex, 0, [ReactionIndex._structure]))
    ...
    def _substructure_unordered(cls, fingerprint: list, substr=None):
        ...
        return RequestPack(request, partial(cls.__postprocess_unordered_cgrs,
                           ...
                           prefetch_map=(
                           ReactionIndex, 0, [ReactionIndex.fingerprint, ReactionIndex._structure]))

    def substructures(self: Union[CGRContainer, "ReactionIndex"], ordered=True, request_only=False):
        if isinstance(self, ReactionIndex):
            fingerprint = self._fingerprint
            mol = self.structure
        elif isinstance(self, CGRContainer):
